...
namespace matplotlibcpp {
    ...
    template<typename Numeric>
    bool imshow(Numeric *ap, int w, int h, const std::string &map) {
        const std::vector <Numeric> a(ap, ap + w * h);

        // using python lists
        PyObject *image = PyList_New(h);
        for (int y = 0; y < h; ++y) {
            PyList_SetItem(image, h - y - 1, PyList_New(w));
            for (int x = 0; x < w; ++x) {
                //*a.at(y*w+x)
                PyList_SetItem(PyList_GetItem(image, h - y - 1), x, PyFloat_FromDouble(1.0 * a.at(y * w + x)));
            }
        ...
        PyDict_SetItemString(kwargs, "cmap", PyString_FromString(map.c_str()));
    ...
    inline void figure() {
        PyObject *res = PyObject_CallObject(detail::_interpreter::get().s_python_function_figure,
